...
class BattleArenaNotifier extends StateNotifier<BattleArenaState> {
  ...
  /// Battle SFX (respects the user's global mute/volume setting from Hive).
  /// Own instance, same pattern as DailyQuizPlayScreen.
  ...
  void submitLocalAnswer(int answerIndex) {
    ...
    try {
      if (isCorrect) {
        ...
        unawaited(_soundService.playWrong());
  ...
  void _handleOpponentForfeited() async {
    ...
    try {
      unawaited(_soundService.playAchievement());
      HapticFeedback.heavyImpact();
    } catch (_) {}
    state = state.copyWith(
      status: BattleArenaStatus.completed,
  ...
  void sendEmote(String emote) {
    ...
    try {
      unawaited(_soundService.playButtonTap());
