...
class _AiChatScreenState extends ConsumerState<AiChatScreen> {
  ...
  final List<Map<String, String>> _quickActions = [
    {
      'label': '📝 Check Grammar',
      'prompt': 'Please check this sentence for grammar errors: '
    },
    {
      'label': '📖 New Word',
      'prompt': 'Teach me a new English word with meaning and example.'
    },
    {
      'label': '💬 Practice',
      'prompt': 'Let\'s do a conversation practice. You start.'
    },
    {
      'label': '✍️ Correct This',
      'prompt': 'Please correct this sentence and explain the grammar rules: '
    },
    {
      'label': '🔊 Pronunciation',
      'prompt':
          'Give me one English sentence to practice pronunciation. Just the sentence, no explanation.'
    },
    {
      'label': '📊 Evaluate',
      'prompt':
          'Please evaluate my English writing. I will paste a paragraph. Give me: Grammar Score (0-100), Vocabulary Level (A1-C2), Fluency Score (0-100), and a corrected version.'
    },
    {
      'label': '🎯 Challenge',
      'prompt':
          'Give me a quick daily English challenge. It should be one of: write 5 sentences in past tense, find errors in a sentence, or describe something. Keep it short and fun!'
    },
    {
      'label': '📝 Exercise',
      'prompt':
          'Generate a fun English exercise for me. Choose one type: Fill in the blanks, Multiple Choice, Error Detection, or Sentence Reordering. Show the exercise first, then provide answers separately.'
    },
  ];
  ...
  final List<Map<String, String>> _lessons = [
    {
      ...
      'desc': '5 min',
      'prompt':
          'We are starting an "Introduce Yourself" lesson. Follow this EXACT format: Ask ONE question. Wait for my answer. If I make mistakes, gently correct me and show the right way. Give brief feedback (1-2 sentences). Then ask the NEXT question. Never answer for me. Never skip ahead. Be patient and encouraging.'
    },
    {
      'id': 'food',
      'icon': '🍽️',
      ...
      'desc': '5 min',
      'prompt':
          'We are starting an "Ordering Food" lesson. You play the waiter, I play the customer. Ask ONE question about my order. Wait for my answer. If my English is wrong, correct me gently and show the proper phrase. Then ask the next question. Never answer for me. Stay in character.'
    },
    {
      'id': 'interview',
      'icon': '💼',
      ...
      'desc': '10 min',
      'prompt':
          'We are starting a "Job Interview" lesson. You are the interviewer. Ask ONE question at a time. Wait for my answer. If I make grammar mistakes, give 1 sentence of feedback. Suggest a better way to say it. Then ask the next question. Keep going for at least 5 questions. Be encouraging.'
    },
    {
      'id': 'routine',
      'icon': '🌅',
      ...
      'desc': '5 min',
      'prompt':
          'We are starting a "Daily Routine" lesson. Ask ONE question at a time about my day. Wait for my answer. If I make mistakes, correct me gently and teach better expressions. If I cannot answer, give me the words I need. Then ask the next question. Cover morning, afternoon, evening.'
    },
    {
      'id': 'past',
      'icon': '📖',
      ...
      'desc': '5 min',
      'prompt':
          'We are starting a "Past Events" lesson. Ask ONE question about my yesterday/weekend. Wait for my answer. If I use wrong tense, say "Good try! We say: [correct sentence]". Explain briefly. Then ask the next question. Do not correct every single mistake, just the important ones.'
    },
  ];
  ...
  void _sendMessage() {
    ...
    if (!_challengeCompleted && _messages.length >= 2) {
      ...
      if (lastAiMsg != null) {
        ...
        if (lastText.contains('challenge') ||
            lastText.contains('write 5') ||
            lastText.contains('find 3 errors')) {
          _completeChallenge();
        }
      }
    }
    ...
    if (_grammarMode) {
      systemPrompt =
          'You are a strict but friendly English grammar teacher named Keshab. '
      ...
      if (_selectedLesson != null) {
        basePrompt +=
            '5. LESSON MODE ACTIVE: You are conducting a structured lesson. '
        ...
        basePrompt +=
            '5. At the end of your response, suggest 2-4 related follow-up questions. '
    ...
    if (systemPrompt != null) {
      ...
      AIService().sendMessage(text).then(addResponse).catchError(handleError);
    }
  }
  ...
  void _simulateTyping(String fullText, List<String> suggestions) {
    ...
    for (int i = 0; i < fullText.length; i++) {
      Future.delayed(Duration(milliseconds: 15 * i), () {
        ...
        // Live auto-scroll during streaming — every 3 chars for smoothness
  ...
  void _detectVocabulary(String text) {
    ...
    final wordPattern =
        RegExp(r"[']([^']+)[']+\s+means?\s+", caseSensitive: false);
  ...
  Future<void> _translateToBangla(String text, Map<String, dynamic> msg) async {
    ...
    try {
      ...
      if (!mounted) return;
      msg['translatedText'] =
          translation ?? '(Could not translate. Please try again.)';
  ...
  void _parseWritingEvaluation(String text) {
    final grammarScore =
        RegExp(r'Grammar\s*Score[:\s]*(\d+)', caseSensitive: false)
            .firstMatch(text);
    final vocabLevel =
        RegExp(r'Vocabulary\s*Level[:\s]*([A-C][12]?)', caseSensitive: false)
            .firstMatch(text);
    final fluencyScore =
        RegExp(r'Fluency\s*Score[:\s]*(\d+)', caseSensitive: false)
            .firstMatch(text);

    if (grammarScore != null || vocabLevel != null) {
      setState(() {
        _writingEvalResult = {
          'grammarScore': grammarScore?.group(1) != null
              ? int.tryParse(grammarScore!.group(1)!) ?? 0
              : null,
          'vocabLevel': vocabLevel?.group(1),
          'fluencyScore': fluencyScore?.group(1) != null
              ? int.tryParse(fluencyScore!.group(1)!) ?? 0
              : null,
        };
      });
    } else {
                      ...
                      SizedBox(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (context, index) {
                            ...
                            final isPronunciation =
                                action['label'] == '🔊 Pronunciation';
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        Expanded(
                            child: _buildPronButton(
                          icon: _userSpeechResult.isNotEmpty
                              ? Icons.check_circle_rounded
                              : Icons.mic_rounded,
                          label:
                              _userSpeechResult.isNotEmpty ? 'Done' : 'Speak',
                          color: _userSpeechResult.isNotEmpty
                              ? Colors.green
                              : Colors.orange,
                          onTap: () => _userSpeechResult.isNotEmpty
                        ...
                        if (_pronunciationAccuracy != null) ...[
                          ...
                          Expanded(
                              child: _buildPronButton(
                            icon: _pronunciationAccuracy! >= 70
                                ? Icons.emoji_events_rounded
                                : Icons.trending_up_rounded,
                            label: '${_pronunciationAccuracy!.toInt()}%',
                            color: _pronunciationAccuracy! >= 70
                                ? Colors.green
                                : Colors.orange,
                            onTap: null,
                          )),
